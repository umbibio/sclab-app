#!/usr/bin/env bash
# SCLab-App pre-uninstall wrapper
# Uses the Python interpreter from the installed prefix to run remove_sclab_app.py
...
PREFIX=${PREFIX:-}
if [[ -z "${PREFIX}" ]]; then
  echo "ERROR: PREFIX is not set. This script must be run by Constructor during uninstall." >&2
  exit 1
fi

LOG_DIR="$PREFIX/var/log/sclab-app"
LOG_FILE="$LOG_DIR/pre_uninstall.log"
mkdir -p "$LOG_DIR"
...
{
  echo "==== SCLab-App pre_uninstall start ===="
  date -u
  echo "PREFIX=$PREFIX"
  ...
  if [[ -x "$PY_UNIX" ]]; then
    ...
    "$PY_UNIX" "$PREFIX/setup/remove_sclab_app.py"
  else
  ...
  echo "==== SCLab-App pre_uninstall end ===="
} | tee -a "$LOG_FILE"
